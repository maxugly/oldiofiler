...
The application, "oldiofiler," is a desktop utility designed for the rapid, manual sorting of audio files. It is written in [Tauri](https://github.com/tauri-apps/tauri)/[Svelte]([https://github.com/tauri-apps/tauri](https://github.com/sveltejs)) It is the original 'old' version of [audiofiler](https://github.com/maxugly/audiofiler), which is being rebuilt in [C++](https://github.com/The-Young-Programer/C-CPP-Programming)/[JUCE](https://github.com/juce-framework/JUCE). The user selects a source directory containing audio files. The application then loads one file at a time, displaying its waveform and allowing the user to play it. Based on the audio content, the user can sort the file into one of several pre-defined subdirectories using single-key shortcuts. The core workflow is optimized for speed and keyboard-based operation, enabling a user to quickly categorize a large queue of files. The application also includes features for customizing the user interface, such as theming and zoom controls.
